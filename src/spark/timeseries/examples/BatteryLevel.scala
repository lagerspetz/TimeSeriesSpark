...
object BatteryLevel{
   ...
   def handlePowerMonitorFile(sc: SparkContext, startDate:Date, startBattery: Int, powerMonitorFile: String, batteryCapacity: Int) = {
    ...
    for (k <- mapped){
      ...
      if (k.length > 2 ){
        ...
        sumV += k(3)
        if (count == span){
          val usage = (k(1)-startTime)*sumA/span/3600
        ...
        //sumV += k(3)
        if (count == span){
          val usage = (k(0)-startTime)*sumA/span/3600
    ...
    if (count < span && count != 0 && mapped.last.length > 2){
      val k = mapped.last
      val usage = (k(1)-startTime)*sumA/span/3600
      values += ((startDate.getTime + (k(1)*1000) toLong, (batteryCapacity - prev - usage)/batteryCapacity*startBattery))
    }else if (count < span && count != 0 && mapped.last.length == 2){
      val k = mapped.last
      val usage = (k(0)-startTime)*sumA/span/3600
      values += ((startDate.getTime + (k(0)*1000) toLong, (batteryCapacity - prev - usage)/batteryCapacity*startBattery))
    }
